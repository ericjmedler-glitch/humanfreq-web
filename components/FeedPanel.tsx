...
type FeedPanelProps = {
  ...
  objectPosition?: string; // CSS objectPosition value, e.g. "center top" — defaults to "center"
...
// any image without washing out the photo. Used on mobile only.
...
// Absolute position of the copy block within the panel on mobile.
...
const storyLinkStyle: React.CSSProperties = {
...
export default function FeedPanel({
  ...
  return (
    ...
    >
      {/* Image cell — on mobile fills 100%, on desktop fills left column */}
      <div className="hf-panel-media">
        ...
        {/* Directional scrim for text legibility on mobile */}
        <div
          className="hf-panel-scrim"
          style={{ position: "absolute", inset: 0, ...scrimStyle(copyPosition, strongScrim) }}
        />

        {/* Mobile-only: copy block and CTA overlaid on image */}
        <div className="hf-panel-mobile-only">
          <div style={textBlockStyle(copyPosition)}>
            <h2 style={headingStyle}>{headline}</h2>
            <p style={bodyStyle}>{body}</p>
            {signature ? <p style={sigStyle}>{signature}</p> : null}
          </div>
      ...
      {/* Desktop-only: right text column */}
        ...
        <div className="hf-panel-desktop-inner">
          ...
          <h2
            style={{
              color: "var(--color-hero-text)",
              fontFamily: "var(--font-cormorant), Georgia, serif",
              fontSize: "clamp(1.85rem, 2.5vw, 2.8rem)",
              fontWeight: 300,
              lineHeight: 1.2,
              margin: "0 0 1rem",
            }}
          ...
          {signature ? (
            <p style={{ ...sigStyle, marginTop: "1.25rem" }}>{signature}</p>
          ) : null}
          {emailCapture ? (
